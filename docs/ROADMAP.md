...
3. **Milestone 2 — Custom places and local business data (draft PR).** Application-owned places,
   OSM provenance, a curated taxonomy, idempotent Windows import, bounded spatial API, native
   category markers, viewport loading, and responsive place details are implemented. Final
   acceptance requires review and physical-iPhone place interaction confirmation.
